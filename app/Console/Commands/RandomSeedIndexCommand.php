...
class RandomSeedIndexCommand extends BaseCommand
{
    ...
    public function handle()
    {
        ...
        $configSeed = $this->getConfig('seed');
        $seedUrl = $configSeed['seed_url'];
        $seedPort = $configSeed['seed_port'];

        $authUrl = $seedUrl . ':' . $seedPort . '/auth/generate';
        ...
        for ($i = 0; $i < $rows; $i++) {
            ...
            $url = $seedUrl . ':' . $seedPort . '/' . $indexName . '/add';
            $request = [
                'headers' => [
                    'Content-Type' => 'application/json',
                    'Accept' => 'application/json',
